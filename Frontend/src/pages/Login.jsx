...
export default function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    if (success) {
      // Yahan apne dashboard ka route daal do
      navigate("/admin"); 
    } else {
      setError("Galat Credentials ya Secret Key!");
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 text-white">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-xl">
        ...
        <p className="text-xs text-slate-400 text-center mb-6">Master Credentials ya Secret Key se login karein</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="text-xs text-slate-300 block mb-1">Password ya Master Key</label>
            <input
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder="Master Secret / Password"
              required
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500"
            />
          </div>
